...
async function changeBasicConnectMemoryVarAddr() {
  ...
  const filePath = path.resolve(__dirname, "../contracts", "common/stores.sol");
  const options = {
    files: filePath,
    from: [
      /return (.*); \/\/ InstaMemory Address/,
      /return (.*); \/\/ InstaEvent Address/,
      /\(model, id\) = (.*); \/\/ connectorID/,
    ],
    to: [
      `return ${memoryInstance.address}; // InstaMemory Address`,
